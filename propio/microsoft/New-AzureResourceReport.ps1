...
$HTML_NSG+="<th align=""center""><font color=""#FFFFFF"">Estado de provisionamiento</font></th>"
...
foreach($INT_TMP_NSG in $DB_AZ_NSG_LST){
    ...
    foreach($INT_TMP_NSG_01 in $INT_TMP_NSG.SecurityRules){
        ...
        $HTML_NSG+="<td align=""center""><font color=""#000000"">$($INT_TMP_NSG.ProvisioningState)</font></td>" 
        ...
        if($INT_TMP_NSG_01.DestinationPortRange -eq '*' -and $INT_TMP_NSG_01.SourceAddressPrefix -eq '*'){
            $INT_CNT_PAD_ALL++
        }
        elseif($INT_TMP_NSG_01.DestinationPortRange -eq '*'){
            $INT_CNT_PRT_ALL++
        }
        elseif($INT_TMP_NSG_01.SourceAddressPrefix -eq '*'){
            $INT_CNT_ADD_ALL++
        }
        $INT_CNT_RUL_ALL++
    }
...
$PRN_PAT = $env:USERPROFILE + "\OneDrive\GitHub\revolver-ps\propio\microsoft\reports\" + (get-Date).ToShortDateString().Replace("/","") + (get-Date).ToShortTimeString().Replace(":","").Replace(" ","") + "-AzureResourcesReport.html"
