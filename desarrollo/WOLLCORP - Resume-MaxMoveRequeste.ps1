do{
    ...
    elseif($MOVE.Status -eq 'Completed'){
        Remove-MoveRequest -Identity $MOVE.Identity -Confirm:$false -Verbose
        Write-Host $Move.Alias 'move migration completed'
    }
    }
    ...
    if($MOVE.Status -eq 'Suspended'){
        if($COUN_INPR -lt $MAX_MOVE){
            ...
            Write-Host 'We started the process for ' $Move.Alias -ForegroundColor Green
            $COUN_INPR++
        }
    ...
    Write-Host "We have 20 move request, so, I go to bed for now...." -ForegroundColor Cyan
    sleep $SLEEP_TIME
