$TMP11 = Get-MSOLUser | Where-Object {$_.UserPrincipalName -like "registro@tecnofor.pe"} | Select DisplayName,UserPrincipalName
$OUserOut = @()
foreach ($RGTMP11 in $TMP11){
        ...
        foreach ($RGTMP22 in $TMP21) {
                            ...
                            $RGTMP31 = [string]$RGTMP22

                            Write-Host $RGTMP31

                            Set-Mailbox $RGTMP11.UserPrincipalName -EmailAddresses @{remove=$RGTMP31}
