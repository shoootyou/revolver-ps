...
##############################################################################################################################
{$CSV = Import-CSV -Delimiter "," -Path ".\admin.csv"
foreach ($Usuario in $CSV)
{
    ...
    $PrimeraLetra = $PrimerNombre.substring(0,1)
    $SAM = $PrimeraLetra + $Usuario.Apellido
    $UPN = $SAM + "@fasscorp.com.pe"
...
{$CSV = Import-CSV -Delimiter "," -Path ".\users.csv"
foreach ($Usuario in $CSV)
{
    $Area = $Usuario.area
    $CodArea = $Area.substring(0,3)
