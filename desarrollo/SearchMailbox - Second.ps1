...
$GBL_DB | Out-File $ENV:USERPROFILE\Desktop\AllUsers-Second.txt
...
foreach($RCV_USR in $GBL_DB){
    ...
    if($NON_EX){
        '----------------------------------------------------------------------------------------------------------' | Out-File $ENV:USERPROFILE\Desktop\DB_Completed-Second.txt -Append
        '|    Working on' + $USR + ' user.' | Out-File $ENV:USERPROFILE\Desktop\DB_Completed-Second.txt -Append
        ...
        Write-Host '----------------------------------------------------------------------------------------------------------' -ForegroundColor Cyan
        '|    Starting proccess at:' + $DAT_START  | Out-File $ENV:USERPROFILE\Desktop\DB_Completed-Second.txt -Append
        '----------------------------------------------------------------------------------------------------------' | Out-File $ENV:USERPROFILE\Desktop\DB_Completed-Second.txt -Append
        try{
            Search-Mailbox $USR_DOM -TargetMailbox $USR_DIS -TargetFolder $REC -LogLevel Full -SearchDumpsterOnly -Confirm:$false -WarningAction SilentlyContinue | Out-File $ENV:USERPROFILE\Desktop\DB_Completed-Second.txt -Append
        ...
        $DAT_FINA = Get-Date
        '----------------------------------------------------------------------------------------------------------' | Out-File $ENV:USERPROFILE\Desktop\DB_Completed-Second.txt -Append
        '|    Proccess finished: ' + $DAT_FINA  | Out-File $ENV:USERPROFILE\Desktop\DB_Completed-Second.txt -Append
        '----------------------------------------------------------------------------------------------------------' | Out-File $ENV:USERPROFILE\Desktop\DB_Completed-Second.txt -Append
        Write-Host '----------------------------------------------------------------------------------------------------------' -ForegroundColor Cyan
        ...
        $USR + ', worked' | Out-File $ENV:USERPROFILE\Desktop\DB_Antiguos-Second.txt -Append

    }
}
pause
#Get-Mailbox -Resultsize unlimited -Filter {RecipientTypeDetails -eq "DiscoveryMailbox"}
#remove-Mailbox gccastaneda-dsmbx -Force -Confirm:$false